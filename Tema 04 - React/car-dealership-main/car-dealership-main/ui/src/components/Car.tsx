...
function Car(props: TProps) {
    ...
    const formatedPrice = new Intl.NumberFormat('en-US', { style: 'currency', currency: 'USD' }).format(car.unitPrice);
    const formatedDiscountPrice = new Intl.NumberFormat('en-US', { style: 'currency', currency: 'USD' }).format(discountPrice);
    const discount = '-' + (car.discountPercentage * 100) + '%';
    const state = { showPopup: false };
    ...
    return (
        <div className="card" style={{ width: '18rem', margin: 20, position: 'relative' }}>
            ...
            <div className="card-body">
                ...
                <a href="#" className="btn btn-primary" onClick={togglePopup}>Buy</a>
            </div>
        </div>
    )
}
